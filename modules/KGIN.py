...
class Aggregator(nn.Module):
    ...
    def forward(self, entity_emb, user_emb, latent_emb,
        ...
        # weight: 不同relation emb的权重，最开始会初始化. [relation-1, dim]
        n_entities = entity_emb.shape[0]
        channel = entity_emb.shape[1]  # dim
        n_users = self.n_users
        n_factors = self.n_factors

        """KG aggregate"""  # 公式10
        head, tail = edge_index
        #  relation emb. 取出对应关系类型的嵌入向量
        edge_relation_emb = weight[edge_type - 1]  # exclude interact, remap [1, n_relations) to [0, n_relations-1)
        # er * ev(tail). er用weight初始化
        neigh_relation_emb = entity_emb[tail] * edge_relation_emb  # [-1, channel]
        # 按照relation连接，聚合、更新item。 公式10
        entity_agg = scatter_mean(src=neigh_relation_emb, index=head, dim_size=n_entities, dim=0)
        # 求用户对intent的注意力分数
        """cul user->latent factor attention"""  # 公式8. latent emb: 不同intent的嵌入表示.[n_factors, channel]
        score_ = torch.mm(user_emb, latent_emb.t())  # [n_user, n_factor]
        score = nn.Softmax(dim=1)(score_).unsqueeze(-1)  # [n_users, n_factors, 1]，β(u,p)
        # score是user和intent之间
        """user aggregate"""  # 公式7
        # adj matrix * entity(item) emb
        user_agg = torch.sparse.mm(interact_mat, entity_emb)  # [n_users, channel]
        # 直接expand是不是没有对每个user的intent进行分析？？disen_weight是intent和relaton之间
        disen_weight = torch.mm(nn.Softmax(dim=-1)(disen_weight_att),  # weight表明KG里relation本身就有权重
                                weight).expand(n_users, n_factors, channel)  # 公式2(softmax)，公式1:e_p
        # score: β(u,p). disen_weight: α(r,p)，没有显式e_r
...
class GraphConv(nn.Module):
    ...
    def __init__(self, channel, n_hops, n_users,
        ...
        # 被封装成一个可训练的 nn.Parameter对象
        self.disen_weight_att = nn.Parameter(disen_weight_att)
        # 将 Aggregator 实例化并将其实例添加到 self.convs 中，这个列表会保存 n_hops 个 Aggregator 层
        for i in range(n_hops):  # context hops
    ...
    def _cul_cor(self):
        ...
        # 计算intent之间的相关性：1.mutual info(公式3); 2.distance(公式4)
    ...
    def forward(self, user_emb, entity_emb, latent_emb, edge_index, edge_type,
        ...
        cor = self._cul_cor()  # 计算intent之间的相似性，返回计算loss
        for i in range(len(self.convs)):
            ...
            # 将每层的输出累加到 entity_res_emb 和 user_res_emb,从而逐层融合图卷积的信息，得到最终的嵌入
            """result emb"""
...
class Recommender(nn.Module):
    def __init__(self, data_config, args_config, graph, adj_mat):
        ...
        self.ind = args_config.ind  # distance计算方式
    ...
    def _init_weight(self):
        ...
        self.latent_emb = initializer(torch.empty(self.n_factors, self.emb_size))  # intent embedding
